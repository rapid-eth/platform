...
var BoxSelector = (_ref) => {
  ...
  (0, _react.useEffect)(() => {
    if (box && props.message && props.message.data) {
      var item = (0, _dotPropImmutableChain.default)(box).get("data.spaces.eth.public.quest").value().filter(i => i.alias === props.message.data);
      console.log(item, 'itsmsm');
      setSelection(item[0]);
    }
  }, [(0, _dotPropImmutableChain.default)(box).get('data').value()]);
  console.log(selection, props, 'sksksksks');
  return selection ? _react.default.createElement(props.component, _objectSpread({}, selection)) : null;
};
