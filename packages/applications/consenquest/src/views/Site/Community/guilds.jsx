  ...
  <Flex minHeight={400}>
    ...
    <Flex column flex={5} p={30}>
      <Flex alignCenter between>
        <Flex alignCenter>
          <Heading mega>Guilds</Heading>
          <Panel content={<CreateGuildPanelForm />} label='Quest Details'>
            <Button xs mx={3}>Create Guild</Button>
          </Panel>
        </Flex>
        <Box card circle boxShadow='blue'>
          <Image src='https://image.flaticon.com/icons/svg/1673/1673589.svg' maxWidth={60} m={3} />
        </Box>
      </Flex>
      <Box>
        <Flex alignCenter>
          <Heading sm heavy noMargin>Featured Guilds: </Heading>
          <Link to={`/quest/kirby`}>
            <Span xxs mx={1} tag='rust'>unity</Span>
          </Link>
          <Link to={`/quest/kirby`}>
            <Span xxs mx={1} tag='rust'>#buidlguidl</Span>
          </Link>
          <Link to={`/quest/kirby`}>
            <Span xxs mx={1} tag='rust'>2cHainZ</Span>
          </Link>
        </Flex>
      </Box>
      <Box mt={4}>
...
const CreateGuildPanelForm = ({ content, styled, ...props}) => { 
 return(
  <Box p={30} width={460}>
    <Heading>Create Guild</Heading>
