...
import { HorizontalRule, Span, Modal, Flex, Heading, Image, BackgroundImage, Paragraph, Box, Container, Button, Card, BackgroundGradient, Panel, Link } from '@horizin/design-system';
    ...
    <Flex minHeight='60vh'>
      <Flex column gradient='gray' flex={2}>
          <Flex center column color='white' bg='purpleDark' gradientDir='145' p={4} minHeight={200}>
            ...
            <Span tag='green' xxs>Bonus DEV Payout</Span>
            <Span my={2}>Make Prediction on Gnosis</Span>
          </Flex>
        <Box p={30}>
          {/* <Heading>Popular Quests</Heading> */}
          <Box textCenter>
      ...
      <Flex column flex={6}>
        ...
        <Flex bg='gray' width='100%' height={70}>
          <Flex center column flex={1} bg='#dc448dcf' color='white'>
        ...
        <Box p={50}>
          <Flex gutter3>
            {
              quests.map( qst => <QuestCardMini alias={qst.alias} slug={qst.alias} {...qst.properties} /> )
            }
          </Flex>
        </Box>
      </Flex>
...
const QuestCardMini = ({
 ...
 return(
  <Flex column p={0}>
    <Flex between column card borderNone flex={1} textAlign='center' p={0} zIndex={20}>
      <Box><Image circle card width={80} maxWidth={150} boxShadow='sunset' mt={-30} src={image} /></Box>
      <Box px={4}>
      ...
      <Flex alignCenter evenly >
        <Panel content={<AddQuestToSapce />} label='Start Quest'>
      ...
      <Flex alignCenter alignSelf='flex-end' between gradient='gray' width='100%' p={3} mt={10} >
        <Flex alignCenter>
...
const AddQuestToSapce = ({ styled, ...props}) => { 
 return(
  <Box width={350} p={4}>
    <Heading md heavy>Add Quest to Space</Heading>
    <Paragraph sm>
      Publically add the quest to your public quest log. Easily move from applications with the same quests.
    </Paragraph>
  </Box>
)
}
